...
fun TareaScreen(
    ...
    LaunchedEffect (tareaId) {
        if (tareaId > 0) {
            viewModel.selectedSystem(tareaId)
        } else {
...
fun TareaEvent(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = {
                    Text(if (uiState.tareaId != null && uiState.tareaId!! > 0) "Editar la tarea" else "Nueva tarea")
                },
                navigationIcon = {
        ...
        Column(
            ...
            if (uiState.tareaId != null && uiState.tareaIdsystemId!! > 0) {
                OutlinedTextField(
